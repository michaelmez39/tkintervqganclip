...
class ReplaceGrad(torch.autograd.Function):
    @staticmethod
    def forward(ctx, forward, backward):
        ctx.shape = backward.shape
        return forward

    @staticmethod
    def backward(ctx, gradients_in):
        return None, gradients_in.sum_to_size(ctx.shape)
...
class Prompt(nn.Module):
    ...
    def forward(self, input):
        input_norm = nn.functional.normalize(input.unsqueeze(1), dim=2)
        embed_norm = nn.functional.normalize(self.embed.unsqueeze(0), dim=2)
        dists = input_norm.sub(embed_norm).norm(dim=2).div(2).arcsin().pow(2).mul(2)
